...
| Attribute extraction | **Three-tier ladder**, best available per image | **Tier 1 — Qwen2-VL-2B** (4-bit NF4, strict-JSON `{garments:[{type,color,color_hex,formality,material}], scene, scene_type, lighting}`, pydantic repair-retry + truncation-salvage): highest quality, ~10–25 s/img on a 4 GB laptop GPU — applied to the demo-critical subset (label pools + supplements). **Tier 2 — CLIP zero-shot inference** (whole corpus, 47 img/s CPU): dominant color via sibling-masked k-means on crop pixels; scene/formality/material zero-shot against vectors the index already stores. **Tier 3 — detector fallback**. All tiers canonicalize into one shared lexicon (24 colors + perceptual-neighbor query expansion, 26 garment types, 7 scene types, 5 formality levels). Production: full-corpus Tier 1 via Bedrock Claude Haiku (one config line) or the one-shot GPU batch job; Moondream2 also drop-in |
...
|---|---|---|
| Bulk-corpus attributes come from CLIP zero-shot (Tier 2), not a VLM | k-means colors drift on dark/low-chroma garments (dark red → maroon; mitigated by neighbor-tolerant query predicates) and formality is fuzzy → prefilter can drop a relevant image (soft-fallback bounds the damage) | full-corpus Tier-1 pass via Bedrock Claude Haiku (~$3, minutes) or the one-shot self-terminating `g4dn` job; a 4 GB shared laptop GPU proved unfit for a 1.2K-image VLM batch (thermal + VRAM-thrash) — that constraint, not the design, set the tier boundary |
